...
fn main() {
    ...
    for game in games.iter() {
        ...
        for info in all_cube_info.iter() {
            ...
            for cube in cube_info.iter() {
                let number_of_cubes: u16 = cube.split(" ").collect::<Vec<&str>>()[0]
                    .parse::<u16>()
                    .unwrap();
        ...
        if valid_game {
            valid_game_ids.push(
                input[0].split(" ").collect::<Vec<&str>>()[1]
                    .parse::<u16>()
                    .unwrap(),
            );
        }

        minimum_cubes_needed.push(
            minimum_cubes_this_cube
                .values()
                .map(|v| *v)
                .collect::<Vec<u16>>(),
        );
    ...
    let power_sum: usize = minimum_cubes_needed
        .iter()
        .map(|v| multiply_elements(v))
        .collect::<Vec<usize>>()
        .iter()
        .sum();
